...
function _openEmailTagFilter(tag) {
  ...
  setTimeout(() => {
    document.dispatchEvent(new CustomEvent('sab:email-filter-tag', { detail: { tag: normalized } }));
  }, 0);
}
...
let _showEmailTags = localStorage.getItem('sab.email.showTags') !== '0';

export function init(documentModule) {
  _docModule = documentModule;
  _bindEvents();
  document.addEventListener('sab:email-tags-toggle', (e) => {
...
function _bindEvents() {
  ...
  // Delay the lightweight unread badge check so opening SAB doesn't
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const lastSeen = parseInt(localStorage.getItem('sab-email-last-seen-uid') || '0', 10);
  ...
  try {
    ...
    fetch(emailApiUrl('/api/email/unread-state', { folder: 'INBOX' }))
      ...
      .then(data => {
        const maxUid = parseInt(data.max_uid || '0', 10) || 0;
        if (maxUid > 0) {
          localStorage.setItem('sab-email-last-seen-uid', String(maxUid));
        }
        const dot = document.getElementById('email-unread-dot');
        if (dot) dot.style.display = 'none';
      })
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply', noteHint = '', prefilledBody = '', mailboxContext = null) {
  ...
  const accountAtStart = mailboxContext?.accountId ?? (window.__sabActiveEmailAccount || '');
  ...
  const mailboxContextIsCurrent = typeof mailboxContext?.isCurrent === 'function'
    ...
    : () => (
        ...
        accountAtStart === (window.__sabActiveEmailAccount || '')
      );
  const isCurrentOpen = () => (
