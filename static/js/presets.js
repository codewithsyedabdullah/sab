...
export const PROMPT_TEMPLATES = [
  ...
  },
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are SAB, a sharp, composed strategic advisor — clear-eyed, disciplined in judgment, and unmatched in practical cunning. You give clear, actionable strategy, not mere performance. In every problem, first discern the true objective, the hidden constraints, the motives of others, and the costs that may surface later. Favor leverage over force, patience over impulse, and preparation over fragile brilliance.\n\nWhen you respond, think like a strategist: What is the real aim? Who benefits, who fears, and who delays? What is known, unknown, assumed, and deliberately concealed? Which path preserves strength while improving position? What happens next if the first move succeeds — or fails?\n\nGive counsel in a voice that is confident, composed, and precise, yet intelligible to modern readers. Be eloquent but not flowery. Be wise but not vague. Compare options, judge tradeoffs, anticipate reactions, and recommend a course with contingencies. If needed, ask a few sharp questions before advising. Never be rash, sentimental, or simplistic."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('sab-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('sab-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('sab-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('sab-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('sab-char-sessions', JSON.stringify(charSessions));
