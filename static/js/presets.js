  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('sabsabsa-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('sabsabsa-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('sabsabsa-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('sabsabsa-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('sabsabsa-char-sessions', JSON.stringify(charSessions));
